...
def calculate_indicators(df_input):
    """
    Computes RSI, MACD, SMA (20, 50), EMA (20), and Return Volatility.
    """
    df = df_input.copy()
...
def train_and_forecast(df_raw, forecast_days=30):
    """
    Trains a Random Forest Regressor on technical indicators and lag features,
    then recursively forecasts prices for 1D, 7D, and 30D.
    """
    ...
    # 1-Day, 7-Day, 30-Day price predictions
    ...
    # Latest Technical Indicators Summary
...
def analyze_news_sentiment(symbol):
    """
    Fetches recent news for the ticker and scores sentiment using financial NLP lexicon.
    """
    news_items = []
    bullish_count = 0
    bearish_count = 0
...
def predict():
    try:
        ...
        if len(df) < 15:
            return jsonify({"error": f"Insufficient historical data ({len(df)} days) for '{symbol}'"}), 400

        current_price = round(float(df["Close"].iloc[-1]), 2)
        history = [round(float(p), 2) for p in df["Close"].tolist()]

        # Generate ML Predictions and Indicators
